...
export class VdpPage {
  ...
  constructor() {
    this.loadingContainer = element(
      by.xpath('//div[@class="loader-container"]')
    );
    this.cashDown = element(
      by.className("ngx-slider-span ngx-slider-pointer ngx-slider-pointer-min")
    );
    this.price = element(
      by.xpath("//div[@class='price']/div[@class='ng-star-inserted']")
    );
    this.annualMileageDefault = element(
      by.xpath('//*[@id="mat-radio-3"]/label/span[2]/span[2]')
    );
    this.creditRatingDefault = element(
      by.xpath(
        "/html/body/main/app-root/mat-drawer-container/mat-drawer-content/div/app-vehicle-details/div[1]/div[2]/div[1]/section/div[1]/vdp-payment-estimation/div/mat-tab-group/div/mat-tab-body[1]/div/app-lease/div/div[3]/app-credit-score/div/div/con-slim-combobox/div/mat-form-field/div/div[1]/div/mat-select/div/div[1]/span"
      )
    );
    this.termsDefault = element.all(
      by.xpath(
        "//mat-radio-group//span[@class='terms-mobile' or @class='terms']"
      )
    );
    this.termSelectedDefault = element(
      by.xpath(
        '(//app-payment-option-list//mat-radio-button//input[@type="radio"])[2]'
      )
    );
    this.tabDefault = element(
      by.xpath(
        '//div[@role="tablist"]//div[@class="mat-tab-label-content"][contains(text(),"Lease")]'
      )
    );
    this.tabFinance = element(
      by.xpath(
        '//mat-tab-header//div[@class="mat-tab-label-content"][contains(text(),"Finance")]'
      )
    );
    ...
    this.creditRatingFinance = element(
      by.xpath(
        '//app-credit-score//mat-select//span[contains(text(),"Excellent Credit (720-850")]'
      )
    );
    ...
    this.paymentOptionsList = element.all(
      by.xpath(
    ...
    this.financeOrOwnButton = element(
      by.css(".trade-in-container .action-buttons .finance-vehicle-button")
    ); //*[@id="trade-in-container"]/div[2]/div[1]
    //kbb modal elements -- start
    this.kbbModal = element(
      by.xpath('//*[@id="kbb-modal-component"]/app-kbb-modal')
    );
    this.kbbModalYear = element(
      by.xpath("//select[@id='Vehicle_Selection_YearId']")
    ); //element(by.css("#Vehicle_Selection_YearId"));
    this.kbbModalBrand = element(
      by.xpath('//select[@id="Vehicle_Selection_ManufacturerId"]')
    );
    this.kbbModalModel = element(
      by.xpath("//select[@id='Vehicle_Selection_ModelId']")
    );
    this.kbbModalStyle = element(
      by.xpath("//select[@id='Vehicle_Configuration_VehicleId']")
    );
    this.kbbModalEngine = element(
      by.xpath("//select[@id='Vehicle_Configuration_EngineId']")
    );
    this.kbbModalTransmission = element(
      by.xpath("//select[@id='Vehicle_Configuration_TransmissionId']")
    );
    this.kbbModalDrivetrain = element(
      by.xpath("//select[@id='Vehicle_Configuration_DrivetrainId']")
    );
    this.kbbModalMileage = element(
      by.xpath("//*[@id='Vehicle_Configuration_Mileage']")
    );
    this.kbbModalZip = element(
      by.xpath("//*[@id='Vehicle_Configuration_ZipCode']")
    );
    this.kbbModalColor = element(
      by.xpath('//select[@id="Vehicle_Configuration_ColorId"]')
    ); //element(by.className('colorSection'));
    this.kbbModalNextButton = element(by.css(".kbbButton1"));
    this.kbbModalNext2Button = element(by.xpath('//*[@id="NextButton"]'));
    this.kbbModalYearSelect = element(
      by.xpath('//*[@id="Vehicle_Selection_YearId"]/option[12]')
    );
    this.kbbModalBrandSelect = element(
      by.xpath('//*[@id="Vehicle_Selection_ManufacturerId"]/option[13]')
    );
    //kbb modal elements -- end

    //this.selectAccessories = element(by.xpath('//*[@id="accessories-summary-container"]/div[2]'));
    this.selectAccessories = element(
      by.xpath("//div[text()=' Select Accessories ']")
    );
    this.accessoriesModal = element(by.className("accessory-model"));
    this.accessoriesViewDetail = element.all(
      by.xpath(
        '//div[@class="accessory-card-view-part"]//span[@class="text-button view-details"][contains(text(),"View details")]'
      )
    );
    this.accessoriesDetailModal = element(
      by.xpath(
        '//accessories-list-model//div[@class="accessory-detail-content"]'
      )
    );
    ...
    this.accessoriesDetailModalSelect = element(
      by.xpath(
        '//span[contains(@class,"primary-button large right-align")][contains(text(),"Remove") or contains(text(),"Select")]'
      )
    );

    // this.accessoriesCardWrapper = element.all(by.className('card-wrapper'));
    this.accessoriesCardWrapper = element.all(
      by.xpath("//div[contains(@class,'card-wrapper')]")
    );
    this.accessoriesCardWrapper_elem = element.all(
      by.xpath("//div[contains(@class,'card-highlight')]")
    );
    // this.accessoriesPrice = element.all(by.className('accessory-price'));
    this.accessoriesPrice = element(
      by.xpath(
        '(//div[@class="accessory-card-price-select"])[1]/span[contains(text(),"$")]'
      )
    );
    this.accessoriesSelectedCount = element(
      by.className("accessory-model-header-selected")
    );
    //   this.accessoriesSelectedTotal = element(by.className('accessory-model-header-total'));
    this.accessoriesSelectedTotal = element(
      by.xpath("//div[contains(@class,'accessory-model-header-total')]")
    );
    ...
    this.accessoriesCheckBox = element.all(
      by.xpath(
        '//div[contains(@class,"accessory-checkbox")]//mat-checkbox//span[@class="mat-checkbox-inner-container"]'
      )
    );

    this.accessoriesDoneButton = element(
      by.xpath(
        '//div//span[@class="accessory-model-header-done"]//span[contains(text(),"Done")]'
      )
    );
    this.accessoriesList = element(by.className("accessory-display"));
    this.accessoriesRemoveAll = element(
      by.xpath(
        '//*[@id="accessories-list-model"]/accessories-list-model/div/div[2]/div/div[2]/span[2]/span'
      )
    );
    this.accessoriesRemove = element(
      by.xpath(
        '//div[@class="accessory-display ng-star-inserted"]//div[text()="Remove"]'
      )
    );
    // this.ppLearnMore = element(by.xpath('//*[@id="product-container"]/div/div[2]/vdp-product-list/div/div[2]/div[1]/span[2]'));
    this.ppLearnMore = element(
      by.xpath(
        '(//vdp-product-list//span[contains(@class,"learn-more-btn")]//a)[last()]'
      )
    );

    //   this.ppModalPlanName = element(by.xpath('//*[@id="protection-product-modal-header"]'));
    this.ppModalPlanName = element(
      by.xpath('//div[@class="protection-product-modal-header"]')
    );

    //  this.ppModalPlanDesc = element(by.xpath('//*[@id="protection-product-text"]/div/div/p'));
    this.ppModalPlanDesc = element(
      by.xpath('//*[@id="protection-product-text"]/div/div')
    );
    //  this.ppModalVideo = element(by.xpath('//*[@id="protection-products-modal-container"]/div[3]/video'));
    this.ppModalVideo = element(
      by.xpath('//div[@class="protection-products-container"]//div//video')
    );

    // this.ppModalCoverageLength = element(by.className('select-content'));
    this.ppModalCoverageLength = element(
      by.xpath(
        '//*[@id="protection-products-modal-component"]/vdp-protection-products-model/protection-products-model-content/div/div[2]/con-slim-combobox/div/mat-form-field/div'
      )
    );

    this.ppModalTotalDue = element(
      by.xpath(
        '//*[@id="protection-products-modal-component"]/vdp-protection-products-model/protection-products-model-content/div/div[2]/div/span[1]'
      )
    );
    ...
    this.ppModalSelectButton = element(
      by.xpath('//div[contains(@class,"select-button")]')
    );
    this.ppModalPlanDetails = element(
      by.xpath(
        '//*[@id="protection-products-modal-component"]/vdp-protection-products-model/protection-products-model-content/div/div[3]/div/div'
      )
    );
    this.ppModalViewBrochure = element(
      by.xpath(
        '//*[@id="protection-products-modal-component"]/vdp-protection-products-model/protection-products-model-content/div/div[4]'
      )
    );
    ...
    this.ppPlanSelectCbClick = element(
      by.xpath(
        '(//vdp-product-list//span[contains(@class,"mat-checkbox-label")])[last()]'
      )
    );
    this.ppPlanSelectCb = element(
      by.xpath(
        '(//vdp-product-list//input[contains(@class,"mat-checkbox-input")])[last()]'
      )
    );
    this.ppPlanSelectCb1 = element(
      by.xpath(
        '(//vdp-product-list//input[contains(@class,"mat-checkbox-input")])[last()]'
      )
    );
    ...
    this.dueAtSigning = element(
      by.xpath('//div[@class="term-length"]/div[2]/span')
    ); //element(by.xpath('/html/body/main/app-root/mat-drawer-container/mat-drawer-content/app-vehicle-details/div/div[2]/div/section[2]/vdp-pricing-summary/div/div[3]/div[1]/div/div[2]/div[2]/span'));
    // this.ppPlanOptionClick = element.all(by.className('product-type'));
    this.ppPlanOptionClick = element(
      by.xpath(
        '(//vdp-product-list//div[contains(@class,"mat-select-arrow")])[last()-2]'
      )
    );
    this.ppPlanOption = element.all(
      by.css("mat-option > .mat-option-text > span")
    );
    ...
    this.paymentOptionRb1 = element(
      by.xpath(
        '(//app-payment-option-list//mat-radio-button//span//input[@class="mat-radio-input"])[last()]'
      )
    );
    ...
    this.unlockSavings = element(
      by.xpath("(//span[contains(text(),'Unlock Savings')])[1]")
    );
    this.extColor = element(
      by.xpath(
        "//div[@class='vehicle-info']//span[normalize-space()='Exterior']"
      )
    );
    this.intColor = element(
      by.xpath(
        "//div[@class='vehicle-info']//span[normalize-space()='Interior']"
      )
    );
    this.engine = element(
      by.xpath(
        "//div[@class='vehicle-info']//div[@class='description limitToTwoLines']"
      )
    );
    this.estMpg = element(
      by.xpath("//span[text()=' est.mpg ']//preceding-sibling::span")
    );
    this.packageAcc = element(by.xpath("//span[contains(text(),'package')]"));
    this.viewVehDetails = element(
      by.xpath("//a[text()='View All Vehicle Details']")
    );
    this.toyotaCareImg = element(by.xpath("//div[@class='care-view']"));
    this.toyotaSafetySense = element(
      by.xpath(
        "//div[@class='vehicle-info-item ng-star-inserted']//a[text()='Toyota Safety Sense']//parent::span//preceding-sibling::img"
      )
    );
    this.carousel = element(by.className("carousel"));
    this.vehDetailModalVin = element(
      by.xpath('//div[@id="vehicle-detail-vin"]')
    );
    this.vehDetailModalVehicleTitle = element(
      by.xpath('//div[@id="vehicle-detail-title"]')
    );
    this.vehDetailModalExtColor = element(
      by.xpath(
        '//div[@class="vehicleDetailRightDescriptionContainer"]/div[1]/div/span[2]'
      )
    );
    this.vehDetailModalIntColor = element(
      by.xpath(
        '//div[@class="vehicleDetailRightDescriptionContainer"]/div[2]/div/span[2]'
      )
    );
    this.vehDetailModalEstMpg = element(
      by.xpath(
        '//div[@class="vehicleDetailLeftDescriptionContainer"]/div[1]/div[2]'
      )
    );
    this.vehDetailModalEngine = element(
      by.xpath(
        '//div[@class="vehicleDetailLeftDescriptionContainer"]/div[2]/div[2]'
      )
    );
    this.vehDetailModalTab1 = element.all(
      by.xpath('//div[@class="mat-tab-labels"]/div[1]')
    );
    this.vehDetailModalTab2 = element.all(
      by.xpath('//div[@class="mat-tab-labels"]/div[2]')
    );
    this.vehDetailModalTab3 = element.all(
      by.xpath(
        "//div[@class='mat-tab-labels']//div[contains(text(),'Safety Features')]"
      )
    );
    this.vehDetailModalArrow = element(
      by.xpath('//div[@class="rightArrowContainer"]')
    );
    this.vehDetailModalTab4 = element.all(
      by.xpath('//div[@class="mat-tab-labels"]/div[4]')
    );
    this.vehDetailModalTab4mobile = element(
      by.xpath(
        "//div[@class='mat-tab-labels']//div[contains(text(),'Package & Accessories')]"
      )
    );
    this.vehDetailModalDisclosure = element(
      by.xpath('//div[@class="Disclosure"]')
    );
    // this.carouselImgRight = element(by.xpath('//img[@alt="arrowRight"]'));
    this.carouselImgRight = element(
      by.xpath(
        '//img[@alt="right arrow"][contains(@class,"arrow-right-small") or contains(@class,"arrow-right-large")]'
      )
    );
    this.additionalDealerSavings = element(
      by.xpath('//div[@class="flexBox AdditionalDealerSavings"]')
    );

    this.confirmAvailabilityForUnlockDealer = element(
      by.xpath(
        '//div[@class="text-button ng-star-inserted"][contains(text()," Confirm Availability ")]'
      )
    );
    this.confirmAvailabilityForNoUnlockDealerOnDesktop = element(
      by.css("button.confirm-availability")
    );
    this.confirmAvailabilityForNoUnlockDealerOnMobile = element(
      by.css(".confirm-availability-btn")
    );
    this.mstcMultiLeadFormModal = element(
      by.className("smart-price-modal-container")
    );
    this.mstcMultiLeadFormModalPaymentTerm = element(
      by.css("div.price-sub-con > .prefix")
    );
    this.mstcMultiLeadFormModalFirstName = element(
      by.xpath(
        '//div[@class="customer-name-field"]/con-textfield[1]/div/mat-form-field/div/div[1]/div[3]/input'
      )
    );
    this.mstcMultiLeadFormModalLastName = element(
      by.xpath(
        '//div[@class="customer-name-field"]/con-textfield[2]/div/mat-form-field/div/div[1]/div[3]/input'
      )
    );
    this.mstcMultiLeadFormModalZipError = element(
      by.css("div.customer-zip-field .mat-error")
    );
    //  this.mstcMultiLeadFormModalTitle = element(by.css('.smart-price-modal-header'));
    this.mstcMultiLeadFormModalTitle = element(
      by.xpath('//span[text()="Availability Requested!"]')
    );
    // this.mstcMultiLeadFormModalReturnToPage = element(by.xpath('//div[@class="return-to-page"]/button'));
    this.mstcMultiLeadFormModalReturnToPage = element(
      by.css(".return-to-page > .primary-button")
    );
    ...
    this.startPurchaseForUnlockDealer = element(
      by.xpath(
        "//div[contains(@class,'price-details')]//button[contains(text(),'Start Purchase')]"
      )
    );
    ...
    this.startPurchaseForNoUnlockDealer = element(
      by.xpath('//button[contains(@class,"start-purchase-bdd")]')
    );
    ...
    this.rightPaneMenu = element(
      by.xpath('//div[@class="menu-items ng-star-inserted"]')
    ); //element(by.css("div.right-pane > div.menu-items"));
    this.signInBtn = element(
      by.xpath("//div[@class='sign-in menu-button ng-star-inserted']")
    ); //element(by.css('.sign-in'));

    this.profileIcon = element(
      by.xpath('//*[@id="dg-component-nav-menu-desktop"]')
    );
    this.zipCodeModal = element(by.xpath("//div[@class='zip-code-container']"));
    this.closeZipCodeModal = element(
      by.xpath('//div[@class="close-btn-container"]')
    );
    ...
    this.zipCodeModalInput = element(
      by.xpath(
        "//mat-dialog-container//mat-form-field//input[contains(@class,'mat-input-element')]"
      )
    );
    this.zipCodeModalDoneBtn = element(
      by.xpath('//mat-dialog-container//button[text()=" Done"]')
    );
    this.zipCodeWarningModal = element(by.id("zipCode-warning-modal-panel"));
    this.zipCodeWarningDoneButton = element(
      by.xpath(
        '//*[@id="zipCode-warning-modal-panel"]/app-zip-code-warning-modal/div[1]/button'
      )
    );
    this.reviewDealZipCode = element(
      by.xpath('//span[@class="text-link custom_zip_button"]')
    );
    ...
    this.applyFinanceModal = element(
      by.className("apply-finance-modal-container")
    );
    this.acceptApplyFinanceModalBtn = element(
      by.className("primary-button large accept_button")
    );
    this.notNowApplyFinanceModalBtn = element(
      by.className("secondary-button large back_button")
    );
    this.authTfsCb = element(
      by.xpath('//div[@class="con-checkbox"]/mat-checkbox')
    );
    this.confirmAndSubmitButton = element(
      by.xpath('//div[@class="finance_buttons"]/button')
    );
    ...
    this.preferredContacts = element.all(
      by.xpath('//div[@class="preferred-contact-selection"]/con-checkbox')
    );
    this.submitToDealerButton = element(
      by.xpath('//div[@class="finance_buttons"]/button')
    );

    this.startPurchaseWaitSpinner = element(by.className("spinner"));
    this.reviewDealPageTitle = element(
      by.className("review-deal-header-content")
    );
  }
}
