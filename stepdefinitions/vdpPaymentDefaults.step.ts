...
if (
  browser.params.browserPlatformCombo === "ChromeDesktop" ||
  browser.params.browserPlatformCombo === "SafariDesktop"
) {
  ...
  Then("User should be navigated to Vehicle Details page", async () => {
    ...
    browser
      .wait(ExpectedConditions.urlContains("vin="), 15000)
  ...
  Then("Cash down is 10% of listed price", async () => {
    ...
    vdpPage.price.getText().then(async (value) => {
      let cprice: number =
        parseInt(value.replace("$", "").replace(",", "")) * 0.1;
  ...
  Then("Default Credit Rating is Excellent", async () => {
    return Assertion.expect(
      vdpPage.creditRatingDefault.getText()
    ).to.eventually.equal("Excellent Credit (720-850)");
  });

  Then("Default Terms are displayed", async () => {
    return vdpPage.termsDefault.each((ele, i) => {
      Assertion.expect(ele.getText()).to.eventually.be.oneOf([
        "24 mos.",
        "36 mos.",
        "48 mos.",
        "60 mos.",
  ...
  Then("Default Term is selected", async () => {
    return Assertion.expect(
      vdpPage.termSelectedDefault.getAttribute("type")
    ).to.eventually.contain("radio");
  });
  ...
  Given("User is in Vehicle Details page", async () => {
    ...
    await waitForVisibilityOf(
      mspFilterPage.sortPriceLowToHigh,
      "sortLowToHigh"
    );
    await mspFilterPage.sortPriceLowToHigh.click();
    await browser.driver.sleep(2 * 1000);
    ...
    await browser.executeScript(
      ...
      mspFilterPage.appcardButton.get(0).getWebElement()
    );
    // browser.executeScript("window.scrollBy(0,250)");
    await waitForVisibilityOf(
      mspFilterPage.appcardButton.first(),
      "appcardButton"
    );
    await mspFilterPage.appcardButton.first().click();
    browser.executeScript(
      "arguments[0].scrollIntoView();",
      mspFilterPage.appCard.get(0).getWebElement()
    );
    // browser.executeScript("window.scrollBy(0,250)");
  ...
  Then("Default Finance Credit Rating is Excellent", async () => {
    return Assertion.expect(
      vdpPage.creditRatingFinance.getText()
    ).to.eventually.equal("Excellent Credit (720-850)");
  });
  ...
  Then("Default Term selected is 60 Months", async () => {
    return Assertion.expect(
      vdpPage.termSelectedFinance.getAttribute("type")
    ).to.eventually.contain("radio");
  });
  ...
  Then(
    "System should call the Payment service and update the payment terms",
    async () => {
      await browser.driver.sleep(10 * 1000);
  ...
  browser.params.browserPlatformCombo === "ChromeAndroid" ||
  browser.params.browserPlatformCombo === "SafariIOS"
) {
  When("User selects a vehicle", async () => {
    await browser.driver.sleep(10 * 1000);
    // vlpFilterPage.appCard.first().click();
  ...
  Then("Default tab is Lease", async () => {
    ...
    //  await browser.driver.sleep(2*1000);
    return Assertion.expect(
      vdpPage.tabDefault.getAttribute("class")
    ).to.eventually.contain("mat-tab-label-content");
  });
