...
When('User clicks on Remove CTA for an accessory', async  () =>{
    await browser.driver.sleep(15*1000);
    //await browser.executeScript("arguments[0].scrollIntoView();",vdpPage.accessoriesRemove);
...
Then('Accessory should be removed from Personalize with Accessories', async () => {
    ...
    return Assertion.expect(vdpPage.selectAccessories.isDisplayed().valueOf()).to.eventually.be.true;
});
